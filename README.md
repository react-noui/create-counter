...
create-counter is a declarative, typescript-driven form control library. create-counter allows you to quickly apply setters/getters/validators within atomic, props-less components by following React's Context API.
...
```typescript
import { createForm } from '@react-noui/create-counter';
...
```typescript
import { createFormUncontrolled } from '@react-noui/create-counter';
const LoginForm = createForm<Login>({
  props: {...}
})
// -> { Context, Provider }
...
https://stackblitz.com/github/react-noui/create-counter
